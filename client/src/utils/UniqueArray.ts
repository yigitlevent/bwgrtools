...
export class UniqueArray<K, T> {
  ...
  **/
  constructor(newArray?: UniqueArrayItem<K, T>[]) {
    if (newArray !== undefined) {
      const remade: UniqueArrayItem<K, T>[] = structuredClone(newArray);
  ...
  **/
  add(item: UniqueArrayItem<K, T>): this {
    ...
    if (index !== undefined) this.values[index] = item;
  ...
  **/
  remove(id: K): this {
    ...
    if (index !== undefined) this.values.splice(index, 1);
    return this;
  }
  ...
  **/
  at(index: number): UniqueArrayItem<K, T> {
    if (index >= this.values.length || this.values.length + index < 0) throw Error("Index out of bounds");
  ...
  **/
  ...
  /**
   * Allows iterating the UniqueArray directly, e.g. `for (const item of uniqueArray)`.
   * @returns `Iterator<UniqueArrayItem<K, T>>`
  **/
  [Symbol.iterator](): Iterator<UniqueArrayItem<K, T>> {
